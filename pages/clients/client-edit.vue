    ...
    <AdminLayout>
        <div id="AdminPage" class="m-4">
            <div class="max-w-[768px] mx-auto">
                ...
                <h1 class="text-2xl font-bold text-center text-gray-900">Client Details for <span class="text-[#61619d]">{{ client.first_name }} {{ client.last_name }}</span></h1>
                ...
                <p class="font-semi-bold italic text-center text-[#a8a8a8] mb-[10px]">As an admin, I can edit my client’s detail</p>
            </div>
        </div>
    </AdminLayout>  
</template>
...
const getClientDetails = async (clientId) => {
    const data = await ClientApi.getClientDetails(id.value, clientId);
    client.value = {
        ...
        hasChecked: data.hasChecked || false, // Assuming there is a 'hasChecked' property
    };
    selectedInterestIds.value = data.client_interests.map(ci => ci.interest_id);
};
...
const updateClient = async () => {
    ...
    try {
        ...
        await ClientApi.updateClient(id.value, route.query.id, updatedClientData);
...
onMounted(async () => {
    await getInterests();
    await getClientDetails(route.query.id);
});
</script>
